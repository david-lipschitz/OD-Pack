unit ODTime;
{
  TODTimePicker & TODDBTimePicker Component
...
type
  TODTimePicker = class(TCustomPanel)
  private
    FTimePanel: TPanel;
  ...
  TODDBTimePicker = class(TODTimePicker)
  private
...
constructor TODTimePicker.Create(AOwner: TComponent);
...
procedure TODTimePicker.UpBtnClick(Sender: TObject);
var
  ed: Tedit;
begin
...
procedure TODTimePicker.DnBtnClick(Sender: TObject);
var
  ed: Tedit;
begin
  if FH2Edit.Focused then ed := FH2Edit
...
procedure TODTimePicker.EditChange(Sender: TObject);
var
  ed: Tedit;
  ch: Char;
begin
...
procedure TODTimePicker.EditKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
    VK_LEFT:
...
procedure TODTimePicker.EditEnter;
begin
  (Sender as TEdit).SelectAll;
end;

procedure TODTimePicker.DoEnter;
begin
  inherited DoEnter;
  FH1Edit.SetFocus;
end;

function TODTimePicker.GetTime: TTime;
begin
...
procedure TODTimePicker.SetTime(Value: TTime);
var
  st: string;
  hh, mm, ss, ms: Word;
begin
...
function TODTimePicker.GetCtl3D: Boolean;
begin
  Result := BorderStyle = bsSingle;
end;

procedure TODTimePicker.SetCtl3D(Value: Boolean);
begin
...
function TODTimePicker.GetShowButtons: Boolean;
begin
  Result := FUpBtn.Visible;
end;

procedure TODTimePicker.SetShowButtons(Value: Boolean);
begin
...
function TODTimePicker.GetEnabled: Boolean;
begin
  Result := inherited Enabled;
end;

procedure TODTimePicker.SetEnabled(Value: Boolean);
begin
...
procedure TODTimePicker.Resize;
var
  wi, ht: Integer;
begin
...
procedure TODTimePicker.DoChange;
begin
  if not (csDesigning in ComponentState) and Assigned(FOnChange) then
    FOnChange(Self);
end;

procedure TODTimePicker.WMMove(var Msg: TWMMove);
begin
  inherited;
...
constructor TODDBTimePicker.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
destructor TODDBTimePicker.Destroy;
begin
  FDataLink.Free;
  inherited Destroy;
end;

procedure TODDBTimePicker.Notification(AComponent: TComponent;
...
procedure TODDBTimePicker.ActiveChange(Sender: TObject);
begin
...
procedure TODDBTimePicker.DataChange(Sender: TObject);
begin
...
procedure TODDBTimePicker.UpdateData(Sender: TObject);
begin
  if FDataLink.Field <> nil then
...
procedure TODDBTimePicker.DoChange;
begin
...
function TODDBTimePicker.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

procedure TODDBTimePicker.SetDataSource(Value: TDataSource);
begin
  FDataLink.DataSource := Value;
  if Value <> nil then Value.FreeNotification(Self);
end;

function TODDBTimePicker.GetDataField: string;
begin
  Result := FDataLink.FieldName;
end;

procedure TODDBTimePicker.SetDataField(const Value: string);
begin
  FDataLink.FieldName := Value;
end;

function TODDBTimePicker.GetField: TField;
begin
  Result := FDataLink.Field;
end;

function TODDBTimePicker.GetTime: TTime;
begin
  Result := inherited Time;
end;

procedure TODDBTimePicker.SetEnabled(Value: Boolean);
begin
  FEnabled := Value;
  inherited Enabled := FEnabled and FDataLink.Active;
